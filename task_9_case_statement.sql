...
create or replace function print_cusine_with_dish(cust_id int, chief int, waiter int, tableno int, dish text, qty int, restaurantid int) 
returns text as $$
	declare
        cusine text;
begin
...
select print_cusine_with_dish(1,2,3,4,'Pizza',6,1);

select print_cusine_with_dish(1,2,3,4,'Pani Puri',6,1);
